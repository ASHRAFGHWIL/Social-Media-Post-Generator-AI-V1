...
import { Moon, Sun, Languages, Sparkles, AlertCircle, Eraser } from 'lucide-react';
import { UI_TEXT, Language, UserInput, GeneratedSocialContent } from './types';
...
const App: React.FC = () => {
  ...
  const handleClear = () => {
    setInput({ description: '', productUrl: '', keyword: '' });
    setResult(null);
    setError(null);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!input.description || !input.keyword || !input.productUrl) {
    ...
    try {
      const data = await generateSocialPosts(input, lang);
      setResult(data);
    } catch (err) {
          ...
          <div className="flex justify-between h-16 items-center">
            ...
            <div className="flex items-center gap-2 md:gap-4">
              ...
              <button 
                ...
                className="p-2 rounded-full hover:bg-gray-100 dark:hover:bg-slate-700 text-gray-600 dark:text-gray-300 transition-colors flex items-center gap-1 font-semibold text-sm"
              >
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                <div>
                  ...
                  <textarea
                    ...
                    rows={4}
                    className="w-full px-4 py-3 rounded-xl bg-gray-50 dark:bg-slate-900 border border-gray-200 dark:border-slate-700 focus:ring-2 focus:ring-brand-500 focus:border-transparent outline-none transition-all resize-none text-gray-900 dark:text-gray-100 placeholder-gray-400"
                    placeholder="..."
                  />
                </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={handleInputChange}
                    className="w-full px-4 py-3 rounded-xl bg-gray-50 dark:bg-slate-900 border border-gray-200 dark:border-slate-700 focus:ring-2 focus:ring-brand-500 focus:border-transparent outline-none transition-all text-gray-900 dark:text-gray-100 placeholder-gray-400"
                    placeholder="https://..."
                  />
                </div>
                ...
                <button
                  type="submit"
                  disabled={loading}
                  className={`w-full py-3.5 px-6 rounded-xl font-bold text-white shadow-lg shadow-brand-500/30 flex items-center justify-center gap-2 transition-all transform active:scale-95
                    ${loading 
          ...
          <div className="lg:col-span-8 xl:col-span-9 space-y-6">
            {!result && !loading && (
              <div className="h-full min-h-[400px] flex flex-col items-center justify-center text-gray-400 border-2 border-dashed border-gray-200 dark:border-slate-700 rounded-2xl bg-white/50 dark:bg-slate-800/30">
                 ...
                 <p className="font-medium">{isRtl ? 'بانتظار المدخلات لبدء السحر...' : 'Waiting for inputs to start the magic...'}</p>
              </div>
            )}

            {result && (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 animate-fade-in">
                <PostCard platform="facebook" content={result.facebook} keyword={input.keyword} lang={lang} />
                <PostCard platform="instagram" content={result.instagram} keyword={input.keyword} lang={lang} />
                <PostCard platform="twitter" content={result.twitter} keyword={input.keyword} lang={lang} />
                <PostCard platform="linkedin" content={result.linkedin} keyword={input.keyword} lang={lang} />
                <PostCard platform="vk" content={result.vk} keyword={input.keyword} lang={lang} />
                <PostCard platform="pinterest" content={result.pinterest} keyword={input.keyword} lang={lang} />
                <PostCard platform="youtube" content={result.youtube} keyword={input.keyword} lang={lang} />
              </div>
            )}
          </div>
